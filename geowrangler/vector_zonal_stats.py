...
def validate_aoi_quadkey(aoi, aoi_quadkey_column) -> None:
    ...
    if aoi_quadkey_column not in list(aoi.columns.values):
        raise ValueError(
            f"aoi_quadkey_column '{aoi_quadkey_column}' is not in list of aoi columns: {list(aoi.column.values)}"
        )
    if len(aoi) == 0:
...
def validate_data_quadkey(data, data_quadkey_column, min_zoom_level):
    if data_quadkey_column not in list(data.columns.values):
        raise ValueError(
            f"data_quadkey_column '{data_quadkey_column}' is not in list of data columns: {list(data.column.values)}"
        )
    if len(data) == 0:
