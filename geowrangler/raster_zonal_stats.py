...
# %% ../notebooks/03_raster_zonal_stats.ipynb #953e0f01
...
# %% ../notebooks/03_raster_zonal_stats.ipynb #6b4dbf4b
...
# %% ../notebooks/03_raster_zonal_stats.ipynb #c22374a2
...
# %% ../notebooks/03_raster_zonal_stats.ipynb #e554b240
...
# %% ../notebooks/03_raster_zonal_stats.ipynb #7b0673e5
def create_exactextract_zonal_stats(
    aoi: Union[  
            str, Path, gpd.GeoDataFrame
    ], # The area of interest geodataframe, or path to the vector file
    data: Union[str, Path],  # The path to the raster data file
    aggregation: Union[Dict, List[Dict[str, Any]]], # Dictionary or list of dictionaries specifying the aggregation
    include_cols: List[str] = None, # If not None, list of columns from input AOI to include in output
    ...
    if "weights" in extra_args:
        warnings.warn("Input weights raster will be used for all passed aggregations. To use different weights for each aggregation, call `create_exactextract_zonal_stats` multiple times.")
    if "include_geom" in extra_args:
        extra_args.pop("include_geom") 
        warnings.warn("include_geom in `extra_args` is ignored. Use `include_geom` in the function parameter instead.")
    if "include_cols" in extra_args:
        extra_args.pop("include_cols") 
        warnings.warn("include_cols in `extra_args` is ignored. Use `include_cols` in the function parameter instead.")
